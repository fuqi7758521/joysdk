...
public class PropertiesUtil {
     ...
     */  
    public static void loadFile(String fileConfig){ //构造方法  
        //获取输入流  
        try {
            String[] files=fileConfig.split(",");
            properties = new Properties();  
            for(String file : files){
                in = PropertiesUtil.class.getClassLoader().getResourceAsStream(file);//这里要说明一下，如果说这个方法是在静态代码块中写的话，那么this是不能用的，this是代表该类的对象，把this改成Object即可，而相应的属性变量也需要改为静态的  
                properties.load(in);  
            }
            in.close();  
        } catch (Exception e) {  
