package com.joysdk.common.datasource;
...
/**
 * <pre>
 *  该类实现了BeanPostProcessor接口，可以对受管理的类进行预处理<br/>
 *  如果这个接口的某个实现类被注册到某个容器，那么该容器的每个受管Bean在调用初始化方法之前，都会获得该接口实现类的一个回调。<br/>
 *  容器调用接口定义的方法时会将该受管Bean的实例和名字通过参数传入方法，进过处理后通过方法的返回值返回给容器。<br/>
 *  根据这个原理，我们就可以很轻松的自定义受管Bean。<br/>
 *   
 *
 * 此类实现了两个职责：
 *   读/写动态数据库选择处理器
 ...
 *       @see me.sunxc.common.datasource.DynamicDataSourceHolder
        ...
        try {
            ...
            for(Entry<String, TransactionAttribute> entry: nameMap.entrySet()) {
                ...
                if(forceChoiceReadWhenWrite) { //不管之前操作是写，默认强制从读库读 （设置为NOT_SUPPORTED即可）
                    ...
                    // 否则 设置为SUPPORTS（这样可以参与到写事务）,如果bean有事务则参与，没有就不启用事务
